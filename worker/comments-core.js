/* ==========================================================================
   余量 MARGIN — 留言板核心逻辑（唯一实现，两处复用）
   --------------------------------------------------------------------------
   为什么单独抽出来：
     同一套接口有两个入口 ——
       1) 独立 Worker        worker/index.js            （workers.dev，备用入口）
       2) Pages Function     functions/api/[[path]].js  （站点自有域名，主入口）
     主入口走 Pages Functions 是因为：同源、不需要 CORS、不依赖 workers.dev
     （workers.dev 在部分网络会被拦截，前端会报 "Failed to fetch"）、
     而且不需要 zone 级的额外权限。
   ...
   绑定
     ...
     ALLOWED_ORIGIN  允许的跨域来源，默认 "*"
                    同源调用（Pages Function）时其实用不到，留着给独立 Worker 入口
   ========================================================================== */
...
/* 两处入口共用这一个处理函数。注意：不依赖 URL 的路径部分，
   所以 /api/?slug=x 与 /?slug=x 行为完全一致。 */
